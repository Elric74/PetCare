...
<template>
  <AppLayout :title="t('items.new_item')">
    <template #header>
        ...
        <form @submit.prevent="submitForm">
          <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
            <div class="sm:col-span-2">
              <label for="item_name" class="block mb-2 text-xs font-medium text-gray-400 dark:text-white">{{ t('items.name') }}</label>
            ...
            <div class="w-full">
              <label for="quantity" class="block mb-2 text-xs font-medium text-gray-400 dark:text-white">{{ t('items.quantity') }}</label>
            ...
            <div class="w-full">
              <label for="unit_price" class="block mb-2 text-xs font-medium text-gray-400 dark:text-white">{{ t('items.unit_price') }}</label>
              <input v-model="createForm.unit_price" type="text" name="unit_price" id="unit_price"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-500"
                :class="errors.unit_price ? 'border-red-500' : ''"
                :placeholder="t('items.unit_price')">
              <span class="text-red-500 text-xs">{{ errors.unit_price }}</span>
            </div>
            <div class="sm:col-span-2">
              <label for="description" class="block mb-2 text-xs font-medium text-gray-400 dark:text-white">{{ t('items.description') }}</label>
