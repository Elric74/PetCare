<?php

namespace Database\Seeders;

use Illuminate\Database\Console\Seeds\WithoutModelEvents;
use Illuminate\Database\Seeder;
use App\Models\Species;
use App\Models\Breed;

class BreedsSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        $breeds = [
            'Cat' => ['Ragdoll', 'Exotic Shorthair', 'British Shorthair', 'Persian cat', 'Maine Coon'],
            'Dog' => [
                'AFFENPINSCHER',
                'AIREDALE TERRIER',
                'AKITA INU',
                'AKITA AMERICAIN',
                'AMERICAN FOXHOUND',
                'AMERICAN STAFFORDSHIRE  TERRIER',
                'ANGLO-FRANÇAIS  DE PETITE VENERIE',
                'ARIEGEOIS',
                'AUSTRALIAN KELPIE',
                'AZAWAKH',
                'BANGKAEW DE THAILANDE',
                'BARBET',
                'BARBU TCHEQUE',
                'BARZOI - LEVRIER DE CHASSE RUSSE',
                'BASENJI',
                'BASSERT ARTESIEN NORMAND',
                'BASSET BLEU DE GASCOGNE',
                'BASSET DE WESTPHALIE',
                'BASSET DES ALPES',
                'BASSET FAUVE DE BRETAGNE',
                'BASSET HOUND',
                'BASSET SUEDOIS',
                'BEAGLE',
                'BEAGLE HARIER',
                'BERGER ALLEMAND',
                'BERGER AUSTRALIEN',
                'BERGER AUSTRALIEN  A QUEUE COURTE',
                'BERGER BERGAMASQUE',
                'BERGER BLANC SUISSE',
                'BERGER D\'ASIE CENTRALE',
                'BERGER DE BEAUCE -- BEAUCERON',
                'BERGER DE BOHEME',
                'BERGER DE BOSNIE-HERZEGOVINE  ET DE CROATIE',
                'BERGER DE BRIE',
                'BERGER DE LA MAREMME ET DES ABRUZZES',
                'BERGER DE PICARDIE-BERGER  PICARD',
                'BERGER DE RUSSIE MERIDIONALE',
                'BERGER DU CAUCASE',
                'BERGER DU KARST',
                'BERGER FINOIS DE LAPONIE',
                'BERGER HOLLANDAIS',
                'BERGER MINIATURE AMERICAIN',
                'BERGER POLONAIS DE PLAINE',
                'BICHON À POIL FRISÉ',
                'BICHON BOLONAIS',
                'BICHON HAVANAIS',
                'BICHON MALTAIS',
                'BILLY',
                'BORDER COLLIE',
                'BORDER TERRIER',
                'BOULEDOGUE ANGLAIS',
                'BOULEDOGUE CONTINENTAL',
                'BOULEDOGUE FRANÇAIS',
                'BOUVIER APPENZELLOIS',
                'BOUVIER AUSTRALIEN',
                'BOUVIER BERNOIS',
                'BOUVIER DE L\'ENTLEBUCH',
                'BOUVIER DES ARDENNES',
                'BOUVIER DES FLANDRES',
                'BOXER',
                'BOYKIN SPANIEL',
                'BRACHET ALLEMAND',
                'BRACHET DE STYRIE A POIL DUR',
                'BRACHET POLONAIS',
                'BRACHET TYROLIEN',
                'BRAQUE ALLEMAND À POIL COURT',
                'BRAQUE D\'AUVERGNE',
                'BRAQUE DE BOURBONNAIS',
                'BRAQUE DE L\'ARIÈGE',
                'BRAQUE DE WEIMAR',
                'BRAQUE FRANÇAIS',
                'BRAQUE HONGROIS',
                'BRAQUE ITALIEN',
                'BRAQUE SAINT-GERMAIN',
                'BRAQUE SLOVAQUE A POIL DUR',
                'BRIQUET GRIFON VENDEEN',
                'BROHOLMER',
                'BUHUND NORVEGIEN',
                'BULL TERRIER',
                'BULLMASTIFF',
                'CAIRN TERRIER',
                'CANICHE',
                'CARLIN',
                'CAVALIER KING CHARLES SPANIEL',
                'CHIEN D\'EAU AMERICAIN',
                'CHIEN À LOUTRE',
                'CHIEN BERGER CROATE',
                'CHIEN CHINOIS À CRÈTE',
                'CHIEN COURANT DE BOSNIE À POIL RAIDE (BARAK)',
                'CHIEN COURANT DE HALDEN',
                'CHIEN COURANT DE LA VALLÉE DE LA SAVE',
                'CHIEN COURANT DE MONTAGNE DU MONTENEGRO',
                'CHIEN COURANT DE SCHILLER',
                'CHIEN COURANT DE TRANSYLVANIE',
                'CHIEN COURANT D\'ISTRIE',
                'CHIEN COURANT DU SMÅLAND',
                'CHIEN COURANT D\'ESTONIE',
                'CHIEN COURANT ESPAGNOL',
                'CHIEN COURANT FILANDAIS',
                'CHIEN COURANT FINLANDAIS',
                'CHIEN COURANT GREC',
                'CHIEN COURANT HAMILTON',
                'CHIEN COURANT ITALIEN A POIL DUR',
                'CHIEN COURANT ITALIEN A POIL RAS',
                'CHIEN COURANT NORVÉGIEN',
                'CHIEN COURANT POLONAIS',
                'CHIEN COURANT SERBE',
                'CHIEN COURANT SLOVAQUE',
                'CHIEN COURANT SUISSE',
                'CHIEN COURANT TRICOLORE SERBE',
                'CHIEN D\'ARRÊT ALLEMAND À POIL DUR',
                'CHIEN D\'ARRÊT ALLEMAND À POIL RAIDE',
                'CHIEN D\'ARRÊT DANOIS ANCESTRAL',
                'CHIEN D\'ARRET FRISON',
                'CHIEN D\'ARRÊT PORTUGAIS',
                'CHIEN D\'ARTOIS',
                'CHIEN DE BALI – KINTAMANI',
                'CHIEN DE BERGER ANGLAIS ANCESTRAL (BOB-TAIL)',
                'CHIEN DE BERGER BELGE',
                'CHIEN DE BERGER CATALAN',
                'CHIEN DE BERGER DE LA SERRA DE AIRES',
                'CHIEN DE BERGER DE MAJORQUE',
                'CHIEN DE BERGER DES PYRENÉES',
                'CHIEN DE BERGER DES SHETLAND',
                'CHIEN DE BERGER DES TATRAS',
                'CHIEN DE BERGER KANGAL',
                'CHIEN DE BERGER ROUMAIN DE BUCOVINE',
                'CHIEN DE BERGER ROUMAIN DE MIORITZA',
                'CHIEN DE BERGER ROUMAIN DES CARPATHES',
                'CHIEN DE BERGER YOUGOSLAVE  DE CHARPLANINA',
                'CHIEN DE CANAAN',
                'CHIEN DE CASTRO LABOREIRO',
                'CHIEN DE COUR ITALIEN',
                'CHIEN DE FERME DANO-SUEDOIS',
                'CHIEN DE GARENNE DES CANARIES',
                'CHIEN DE GARENNE PORTUGAIS',
                'CHIEN DE LA SERRA DA ESTRELA',
                'CHIEN DE LEONBERG',
                'CHIEN DE MONTAGNE DE L\'ATLAS (AÏDI)',
                'CHIEN DE MONTAGNE DES PYRENÉES',
                'CHIEN DE PERDRIS DE DRENTE',
                'CHIEN DE RECHERCHE AU SANG DE HANOVRE',
                'CHIEN DE RHODESIE A CRETE DORSALE - RHODESIAN RIDGEBACK',
                'CHIEN DE SAINT-HUBERT',
                'CHIEN DE TAIWAN',
                'CHIEN D\'EAU AMÉRICAIN',
                'CHIEN D\'EAU ESPAGNOL',
                'CHIEN D\'EAU FRISON',
                'CHIEN D\'EAU IRLANDAIS',
                'CHIEN D\'EAU PORTUGAIS',
                'CHIEN D\'EAU ROMAGNOL',
                'CHIEN D\'ELAN NORVEGIEN (Gris – Noir)',
                'CHIEN D\'ÉLAN SUÉDOIS',
                'CHIEN D\'OURS DE CARELIE',
                'CHIEN D\'OYSEL ALLEMAND',
                'CHIEN DU GROENLAND',
                'CHIEN DU MONT SAINT-BERNARD',
                'CHIEN DU PHARAON',
                'CHIEN ESQUIMAU CANADIEN',
                'CHIEN FINNOIS DE LAPONIE',
                'CHIEN LOUP TCHÉCOSLOVAQUE',
                'CHIEN NOIR ET FEU POUR LA CHASSE AU RATON LAVEUR',
                'CHIEN NORVÉGIEN DE MACAREUX',
                'CHIEN NU DU PEROU',
                'CHIEN NU MEXICAIN',
                'CHIEN ROUGE DE BAVIÈRE',
                'CHIEN THAILANDAIS A CRETE DORSALE - THAILAND-RIDGEBACK',
                'CHIEN-LOUP DE SAARLOOS',
                'CHIENS DE BERGER ISLANDAIS',
                'CHIHUAHUA (Poil long – Poil court)',
                'CHOW-CHOW',
                'CIMARRON URUGUAYEN',
                'CIRNECO DE L\'ETNA',
                'CLUMBER SPANIEL',
                'COCKER AMÉRICAIN',
                'COCKER SPANIEL ANGLAIS',
                'COLLIE',
                'COLLIE BARBU',
                'COTON DE TULEAR',
                'DALMATIEN',
                'DANDIE DINMONT TERRIER',
                'DEERHOUND',
                'DEUTSCH LANGHAAR',
                'DOBERMANN',
                'DOGUE ALLEMAND',
                'DOGUE ARGENTIN',
                'DOGUE DE BORDEAUX',
                'DOGUE DE MAJORQUE',
                'DOGUE DES CANARIES',
                'DOGUE DU TIBET',
                'DRENTSCHE PATRIJSHOND',
                'ENGLISH FOXHOUND',
                'ENGLISH SPRINGER SPANIEL',
                'EPAGNEUL BLEU DE PICARDIE',
                'EPAGNEUL BRETON',
                'EPAGNEUL DE PONT-AUDEMER',
                'EPAGNEUL FRANÇAIS',
                'EPAGNEUL JAPONAIS',
                'EPAGNEUL KING CHARLES',
                'EPAGNEUL NAIN CONTINENTAL Papillon',
                'EPAGNEUL PICARD',
                'EPAGNEUL TIBETAIN',
                'EURASIER',
                'FIELD SPANIEL',
                'FILA BRASILEIRO',
                'FILA DE SAINT MIGUEL',
                'FOX TERRIER (Poil lisse – Poil Dur)',
                'FOXHOUND AMERICAIN',
                'FRANÇAIS BLANC ET NOIR',
                'FRANÇAIS BLANC ET ORANGE',
                'FRANÇAIS TRICOLORE',
                'GASCON SAINTONGEOIS',
                'GOLDEN RETRIEVER',
                'GRAND ANGLO-FRANÇAIS  BLANC ET NOIR',
                'GRAND ANGLO-FRANÇAIS  BLANC ET ORANGE',
                'GRAND ANGLO-FRANÇAIS  TRICOLORE',
                'GRAND BASSET GRIFFON VENDEEN',
                'GRAND BLEU DE GASCOGNE',
                'GRAND BOUVIER SUISSE',
                'GRAND EPAGNEUL DE MÜNSTER',
                'GRAND GRIFFON VENDEEN',
                'GRAND SAINTONGEOIS',
                'GRIFFON (Griffon Bruxellois – Griffon Belge – Petit Brabançon)',
                'GRIFFON À POIL DUR KORTHALS',
                'GRIFFON BLEU DE GASCOGNE',
                'GRIFFON FAUVE DE BRETAGNE',
                'GRIFFON NIVERNAIS',
                'HARRIER',
                'HOKKAIDO',
                'HOVAWART',
                'HUSKY DE SIBÉRIE',
                'JINDO COREEN',
                'KAI',
                'KELPIE AUSTRALIEN',
                'KISHU',
                'KOMONDOR',
                'KROMFOHRLÄNDER',
                'KUVASZ',
                'LHASA APSO',
                'LAIKA DE SIBERIE OCCIDENTALE',
                'LAIKA DE SIBERIE ORIENTALE',
                'LAIKA RUSSO-EUROPEEN',
                'LAKELAND TERRIER',
                'LANCASHIRE HEELER',
                'LANDSEER',
                'LAPPHUND SUEDOIS',
                'LEVRETTE D\'ITALIE',
                'LEVRIER AFGHAN',
                'LEVRIER ANGLAIS',
                'LEVRIER ESPAGNOL',
                'LEVRIER HONGROIS',
                'LEVRIER IRLANDAIS',
                'LEVRIER POLONAIS',
                'MALAMUTE D\'ALASKA',
                'MASTIFF',
                'MATIN DES PYRENÉES',
                'MATIN ESPAGNOL',
                'MATIN NAPOLITAIN',
                'MATIN TRANSMONTANO',
                'MUDI',
                'NEDERLANDSE  KOOIKERHONDJE',
                'NORFOLK TERRIER',
                'NORWICH TERRIER',
                'PEKINOIS',
                'PERDIGUERO DE BURGOS',
                'PETIT BASSET GRIFFON VENDEEN',
                'PETIT BLEU DE GASCOGNE',
                'PETIT CHIEN COURANT SUISSE',
                'Chien courant Bernois',
                'Chien courant du Juras',
                'Chien courant lucernois',
                'Chien courant de Schwyz',
                'PETIT CHIEN LION',
                'PETIT CHIEN RUSSE',
                'PETIT EPAGNEUL DE MÜNSTER',
                'PINSCHER ALLEMAND',
                'PINSCHER AUTRICHIEN',
                'PINSCHER NAIN',
                'PISTEUR BRÉSILIEN',
                'PLOTT HOUND',
                'PODENCO D\'IBIZA',
                'POINTER ANGLAIS',
                'POINTER PORTUGAIS',
                'POITEVIN',
                'PORCELAINE',
                'PRESA CANARIO',
                'PUDELPOINTER',
                'PULI',
                'PUMI',
                'RAFEIRO DE L\'ALENTEJO',
                'RAFEIRO DE L\'ALTENTEJO',
                'RATIER VALENCIEN',
                'RAT TERRIER',
                'RETRIEVER À POIL BOUCLÉ',
                'RETRIEVER À POIL PLAT',
                'RETRIEVER DE LA BAIE DE CHESAPEAKE',
                'RETRIEVER DE LA NOUVELLE-ECOSSE',
                'RETRIEVER DU LABRADOR',
                'ROTTWEILER',
                'SALUKI (Poil frangé – Poil ras)',
                'SAMOYEDE',
                'SCHAPENDOES  NÉERLANDAIS',
                'SCHIPPERKE',
                'SCHNAUZER',
                'SEALYHAM TERRIER',
                'SETTER ANGLAIS',
                'SETTER GORDON',
                'SETTER IRLANDAIS',
                'SHAR PEI',
                'SHIBA',
                'SHIH TZU',
                'SHIKOKU',
                'SKYE TERRIER',
                'SLOUGHI',
                'SMOUS DES PAYS-BAS',
                'SPINONE',
                'SPITZ ALLEMAND',
                'Spitz loup',
                'Grand Spitz',
                'Spitz moyen',
                'Petit Spitz',
                'Spitz nain',
                'SPITZ DE NORRBOTTEN',
                'SPITZ FINLANDAIS',
                'SPITZ JAPONAIS',
                'STAFFORDSHIRE  BULL TERRIER',
                'SUSSEX SPANIEL',
                'TCHOUVATCH SLOVAQUE',
                'TECKEL',
                'TEDDY ROOSEVELT TERRIER',
                'TERRE-NEUVE',
                'TERRIER ANGLAIS D\'AGRÉMENT',
                'TERRIER AUSTRALIEN À POIL SOYEUX',
                'TERRIER AUSTRALIEN',
                'TERRIER BRESILIEN',
                'TERRIER DE BEDLINGTON',
                'TERRIER DE BOSTON',
                'TERRIER DE CHASSE ALLEMAND',
                'TERRIER DE MANCHESTER',
                'TERRIER DU REVEREND RUSSELL',
                'TERRIER DU YORKSHIRE',
                'TERRIER ECOSSAIS',
                'TERRIER IRLANDAIS',
                'TERRIER IRLANDAIS À POIL DOUX',
                'TERRIER IRLANDAIS GLEN OF IMAAL',
                'TERRIER JACK RUSSELL',
                'TERRIER JAPONAIS',
                'TERRIER KERRY BLUE',
                'TERRIER NOIR RUSSE',
                'TERRIER TCHÈQUE',
                'TERRIER TIBETAIN',
                'TOSA',
                'VALLHUND SUEDOIS - SPITZ DES VISIGOTHS',
                'VOLPINO ITALIEN',
                'WELSH CORGI CARDIGAN',
                'WELSH CORGI PEMBROKE',
                'WELSH SPRINGER SPANIEL',
                'WELSH TERRIER',
                'WEST HIGHLAND WHITE TERRIER',
                'WHIPPET',
            ],
            'Bird' => ['Ostrich', 'Rhea', 'Cassowary', 'Emu', 'Kiwi'],
            'Rabbit' => ['American', 'Argente Brun', 'Argente de Champagne', 'Argente Noir', 'Argente St Hubert'],
            'Fish' => ['Swordfish', 'Atlantic cod', 'Mackerel', 'Trout', 'Atlantic salmon'],
            'Reptile' => ['American alligator', 'Gopher tortoise', 'California kingsnake', 'Gila monster', 'Dorset sheep'],
            'Horse' => ['American Milking Devon', 'Anatolian Black', 'Andalusian Black', 'Angus', 'Ankole-Watusi'],
            'Cow' => ['American Angus', 'American Brahman', 'American Breed', 'American Milking Devon', 'American White Park'],
            'Sheep' => ['Bannur Sheep', 'Barbados Black Belly', 'Cheviot Sheep', 'Columbia Sheep', 'Corriedale Sheep'],
            'Goat' => ['Alpine Goat', 'Belgian Fawn Goat', 'LaMancha Goat', 'Nigerian Dwarf Goat', 'Oberhasli Goat'],
            'Pig' => ['American Yorkshire', 'Angeln Saddleback', 'Appalachian English', 'Arapawa Island', 'Arikara'],
            'Chicken' => ['Australorp', 'Barnevelder', 'Brahma', 'Cochin', 'Leghorn'],
            'Duck' => ['Abacot Ranger', 'Aylesbury', 'Bali', 'Black East Indian', 'Blue Swedish'],
            'Turkey' => ['Auburn', 'Black', 'Bourbon Red', 'Bronze', 'Narragansett'],
            'Guinea Pig' => ['Abyssinian', 'American', 'Coronet', 'Peruvian', 'Silkie'],
            'Hamster' => ['Campbell\'s Dwarf', 'Chinese', 'Roborovski', 'Syrian', 'Winter White Russian Dwarf'],
            'Ferret' => ['Albino', 'Black Sable', 'Champagne', 'Chocolate', 'Sable'],
            'Chinchilla' => ['Standard Gray', 'Beige', 'Black Velvet', 'Violet', 'White'],
            'Parrot' => ['African Grey', 'Amazon', 'Budgerigar', 'Cockatiel', 'Cockatoo'],
            'Turtle' => ['African Aquatic Sideneck', 'Asian Box', 'Eastern Box', 'Mississippi Map', 'Russian']
        ];

        // Map species names to their French translations
        $speciesMap = [
            'Cat' => 'Chat',
            'Dog' => 'Chien',
            'Bird' => 'Oiseau',
            'Rabbit' => 'Lapin',
            'Fish' => 'Poisson',
            'Reptile' => 'Reptile',
            'Horse' => 'Horse',
            'Cow' => 'Cow',
            'Sheep' => 'Sheep',
            'Goat' => 'Goat',
            'Pig' => 'Pig',
            'Chicken' => 'Chicken',
            'Duck' => 'Duck',
            'Turkey' => 'Turkey',
            'Guinea Pig' => 'Guinea Pig',
            'Hamster' => 'Hamster',
            'Ferret' => 'Ferret',
            'Chinchilla' => 'Chinchilla',
            'Parrot' => 'Parrot',
            'Turtle' => 'Turtle'
        ];

        foreach ($breeds as $speciesName => $breedList) {
            $translatedName = $speciesMap[$speciesName] ?? $speciesName;
            $species = Species::where('name', $translatedName)->first();

            if ($species) {
                foreach ($breedList as $breedName) {
                    Breed::create([
                        'species_id' => $species->id,
                        'name' => $breedName
                    ]);
                }
            }
        }
    }
}
